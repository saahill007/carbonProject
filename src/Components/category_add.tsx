...
const Category_add: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosInstance.get<Category[]>("/api/category_add");
  ...
  const handleSave = async () => {
    ...
    if (!newCategory.category_name.match(onlyLetters)) {
      ...
      if (
        lowerCaseData.some(
          (category) => category.category_name === lowerCaseNewCategoryName
        )
      ) {
  ...
  return (
    <div style={{ paddingLeft: "100px", paddingTop: "70px" }}>
      ...
      <button className="back_option" onClick={handlecategory}>
        Back
      </button>
