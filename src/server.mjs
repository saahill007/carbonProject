...
app.post("/api/slider", cors(), (req, res) => {
  ...
  const sql =
    'INSERT INTO CarbnOffset.questions (questions, question_flag, type_of_question) VALUES (?, 1, "Slider")';
...
app.post("/api/dropdown", cors(), (req, res) => {
  ...
  mysqlConnection.query(insertQuestionSql, [question], (err, results) => {
    ...
    if (results.affectedRows === 1) {
      ...
      // Insert options into the "Options" table with queId as a foreign key
      const insertOptionsSql =
        "INSERT INTO CarbnOffset.Options (ques_id, given_option, option_type, equivalent_carbon) VALUES ?";
      const optionsData = options.map((option) => [
        queId,
        option.dropdown,
        "Dropdown",
